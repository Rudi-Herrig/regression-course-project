...
## Based on the feedback received from the previously submitted Project Paper 1, we have made the following changes: Changed our Response Variable from the binary variable County Borders Navigable Waterway (ACX001) to the Illiteracy Rate, which we calculate by dividing the illiterate population by the total White population.


continuous variable People Employed in Selected Industries (ADE011). We have also added a new Response Variable, Capital Invested (ADC011). We have also added a new Predictor
...
```{r}
# png("ADD006hist2.png")
hist(Persons_Employed_Inland_Navigation, main = "Persons Employed in Inland Navigation", xlab = "Persons Employed in Inland Navigation", ylab = "Number of Counties", col = "lightblue", xlim=c(0,600), breaks=100)
# dev.off()
```
...
table(Manufacturing_Establishments_Fisheries)
```
...
```{r}
hist(Manufacturing_Establishments_Fisheries, main = "Manufacturing Establishments: Fisheries", xlab = "Manufacturing Establishments: Fisheries (Capital invested in Millions of $USD)", ylab = "Number of Counties", col = "lightblue", xaxt = "n")
axis(1, at = breaks, labels = paste0(breaks/1e3, "k"))
Manufacturing_Establishments_Fisheries_ZerosRemoved <- Manufacturing_Establishments_Fisheries[Manufacturing_Establishments_Fisheries != 0]
hist(Manufacturing_Establishments_Fisheries_ZerosRemoved, main = "Manufacturing Establishments: Fisheries", xlab = "Manufacturing Establishments: Fisheries (Capital invested in Millions of $USD)", ylab = "Number of Counties", col = "lightblue", breaks= 100)

# Define custom bins
breaks <- c(0, 100, 500, 1000, 5000, 10000, 50000, Inf)
labels <- c("$0", "$1-$100", "$101-$500", "$501-$1,000", "$1,001-$5,000", "$5,001-$10,000", ">$10,000")

# Categorize data
categorized_data <- cut(Manufacturing_Establishments_Fisheries, breaks=breaks, labels=labels, include.lowest=TRUE)

# Calculate the counts for each bin
counts <- table(categorized_data)

# Set parameters for slanted text on x-axis labels
par(las=1) # Always set to 1 for horizontal labels if using srt
par(mar=c(7, 4, 4, 2) + 0.1) # Adjust the margin to make sure labels fit

# Create bar plot
png("ADC014CatBarPlot.png")
barplot(counts, main="Categorized Capital Investment in Fisheries",
        ylab="Number of Counties",
        col="lightblue", cex.names=0.65, srt=45)
dev.off()
# Reset par to default values after plotting
...
bigboynav[774,]
Flour_Barrels_Manufactured_2 <- Flour_Barrels_Manufactured[-774]
max(Flour_Barrels_Manufactured_2)
which.max(Flour_Barrels_Manufactured_2)
bigboynav[787,]
```
...
```{r}
cat(cor(County_Access_Navigable_Waterway, Persons_Employed_Inland_Navigation), ": Persons_Employed_Inland_Navigation's correlation coefficient with our Response", "\n")
cat(cor(County_Access_Navigable_Waterway, Manufacturing_Establishments_Fisheries), ": Manufacturing_Establishments_Fisheries's correlation coefficient with our Response:", "\n")
cat(cor(County_Access_Navigable_Waterway, Forest_Products_Tar_Pitch_Terpentine_Resin), ": Forest_Products_Tar_Pitch_Terpentine_Resin's correlation coefficient with our Response:", "\n")
cat(cor(County_Access_Navigable_Waterway, Flour_Barrels_Manufactured), ": Flour_Barrels_Manufactured's correlation coefficient with our Response:", "\n")
```
The correlation between the response variable and the predictor variables is weak, with the highest correlation being 0.229782. This is not a good sign for our model, as we want to see a strong correlation between the predictor variables and the response variable. This may be because of the distribution of our response, or the simple variability between counties.
Let's create a correlation matrix for a better visualization.
```{r}
# I like the way this output is formatted
print({m <- round(cor(modelvariables), 4); diag(m) <- NA; m}, na.print = "")
print({m <- round(cor(modelvariables[,2:5]), 4); diag(m) <- NA; m}, na.print = "")
```
```{r}
round(modelvariables %>%
  select(-ACX001) %>%
  cor(), 3)
```
```{r}
cor(modelvariables[,2:5])
```
...
# Removing extreme outliers using the IQR method (Q1 - (1.5 * IQR), Q3 + (1.5 * IQR))
```{r}
# Function to remove outliers and display the range
...
# Removing outliers from each column individually and displaying the range
cat("Persons_Employed_Inland_Navigation:\n")
Persons_Employed_Inland_Navigation_OutliersRemoved <- remove_outliers(bigboynav$ADD006)

cat("\nManufacturing_Establishments_Fisheries:\n")
Manufacturing_Establishments_Fisheries_OutliersRemoved <- remove_outliers(bigboynav$ADC014)

cat("\nForest_Products_Tar_Pitch_Terpentine_Resin:\n")
Forest_Products_Tar_Pitch_Terpentine_Resin_OutliersRemoved <- remove_outliers(bigboynav$ADB004)

cat("\nFlour_Barrels_Manufactured:\n")
Flour_Barrels_Manufactured_OutliersRemoved <- remove_outliers(bigboynav$ADB013)
```
